...
import pygame
import sys
from pygame.locals import QUIT, KEYDOWN, K_UP, K_DOWN, K_SPACE, K_a
...
waiting_music = 'tenefo-73435.mp3'  # 대기 화면 음악
missile_sound = 'positive_beeps-85504.mp3'   # 미사일 발사 소리
game_over_sound = 'game-over-160612.mp3'   # 게임 오버 소리
hit_sound = 'big-hit-sound-effect-241416.mp3'   # 장애물 맞췄을 때 소리
...
class Draw:
    ...
    # 타원을 그리는 함수
    def draw_E(self):
        pygame.draw.ellipse(SURFACE, self.rect, self.col)

    # 사각형을 그리는 함수
    def draw_R(self):
        pygame.draw.rect(SURFACE, self.rect, self.col)
...
def main():
    ...
    # 플레이어 캐릭터와 미사일 설정
    Cir = Draw((255, 255, 255), pygame.Rect(50, 300, 30, 30))
    Beam = Draw((255, 255, 0), pygame.Rect(Cir.rect.centerx, Cir.rect.centery, 5, 5), 10)
    ...
    while True:
        ...
        if game_start:
            ...
            # 장애물이 없으면 새로 생성
            if len(Rock) == 0:
                Rock.append(Draw((0, 255, 0), pygame.Rect(xpos, ypos * 40 + 170, RockWIDTH - ypos * 3, RockHEIGHT - ypos * 3), rock_speed))
            elif len(Rock) == 1:
